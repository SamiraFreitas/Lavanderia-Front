...
export function ReadAndExcludeClient() {
  ...
  const [clientes, setClientes] = useState(new ClienteAPI());
	...
	async function handleToEditClient() {
		...
		history.push(`/edit/client/${rowsSelected[0].cpf}`);
	}
				...
				<Modal
					...
					handleToConfirm={() => {
						...
						for (var el in rowsSelected) {
							clientes.deleteCliente(rowsSelected[el].cpf);
						}
						setExcludedClient(true);
					}}>
					{`Deseja excluir o(s) cliente(s) ${rowsSelected.map(
						(e) => " " + e.nome
					)} ?`}
				</Modal>
			) : (
